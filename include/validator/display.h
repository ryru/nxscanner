...
class Display {
 ...
 public:
  Display(std::ostream &out, std::vector<nxs::scanner::Probes> &allProbes) : out{out}, allProbes{allProbes} {}
  void print();
 ...
 private:
  std::ostream &out;
  const std::vector<nxs::scanner::Probes> &allProbes;
  uint32_t domainTotal{0};
  uint32_t blockedDomainTotal{0};

  void printBlockedDomain(nxs::scanner::Probes const &p) const;
  void printNonBlockedDomain(nxs::scanner::Probes const &p) const;
  void printNonResolvableDomain(nxs::scanner::Probes const &p) const;

...
std::vector<nxscan::scanner::Resolver> prepareScan(std::vector<nxscan::scanner::Probes> &allProbes,
  ...
  std::for_each(allProbes.begin(), allProbes.end(), [&](nxscan::scanner::Probes &p) {
    allResolvers.emplace_back(io_service1, p.getRandomHost());
    allResolvers.emplace_back(io_service2, p.getHost());
    allResolvers.emplace_back(io_service3, p.getRandomHost());
  });
  return allResolvers;
}
